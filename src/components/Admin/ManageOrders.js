    ...
    <div className="sm:p-5 rounded-lg shadow-lg">
      {data?.length > 0 ? (
        data?.map((order, index) => {
          return (
            <div
              className="relative sm:h-fit mb-12 sm:mb-8"
              key={index}
            >
              ...
              <div
                key={index}
                className="sm:rounded-none bg-[#f7e2e2] shadow-sm sm:shadow-none sm:bg-white sm:absolute sm:top-[50%] sm:left-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] p-4 md:flex w-full sm:w-[95%] sm:h-[90%]"
              >
        ...
        <div className="p-5">Currently, you haven't been any orders yet!</div>
      )}
    </div>
  );
};
